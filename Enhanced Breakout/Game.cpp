...
void Game::Init()
{
    ...
    // load textures
    ResourceManager::LoadTexture("../textures/ezra.png", true, "face");
...
void Game::DoCollisions()
{
    for (GameObject& box : this->Levels[this->Level].Bricks)
    {
        if (!box.Destroyed)
        {
            ...
            if (std::get<0>(collision)) // if collsion is true
            {
...
bool CheckCollision(GameObject& one, GameObject& two) // AABB - AABB collision
{
    ...
    // collision ony if collision on both axes
...
Collision CheckCollision(BallObject& one, GameObject& two) // AABB - circle collision
{
    ...
    // add clamped value to AABB_center and we get the point on the edge of the box that is closest
    // to the circle
    glm::vec2 closest = aabb_center + clamped;
    // retrieve vector between center circle and closest point of AABB and check if length <= radius
...
// calculates which direction a vector is facing (N, S, E or W)
